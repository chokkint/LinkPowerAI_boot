		...
		e.event.special.ace_drag = {
			setup : function(){
				...
				n.on(a, function(a){
					...
					c = {coords : [l.pageX, l.pageY], origin : e(a.target)};
...
ace.handle_side_menu = function(e){
	...
	e(document).on(ace.click_event + ".ace.menu", "#menu-toggler", function(){
		...
		e(this).hasClass("display") && "sidebar_scroll" in ace.helper && ace.helper.sidebar_scroll.reset(), !1;
		...
		s = t.attr("data-icon1"), i = t.attr("data-icon2");
	...
	e(document).on(ace.click_event + ".ace.submenu", ".sidebar .nav-list", function(a){
		...
		if(i && 0 != i.length){
			...
			if(i.hasClass("dropdown-toggle")){
				...
				if(n && c == s || e(r.parentNode).hasClass("hover") && !ace.vars.collapsible)
			...
			if("tap" == ace.click_event && n && i.get(0).parentNode.parentNode == s){
				...
				if(a.target != h && !e.contains(h, a.target)){
					return a.preventDefault(), !1;
				}
			}
			if(t && "false" !== i.attr("data-link")){
				return document.location = i.attr("href"), a.preventDefault(), !1;
			}
		}
	});
},
...
ace.submenu = {
	show : function(e, a){
		var t, s = $(e);
		if(s.trigger(t = $.Event("show.ace.submenu")), t.isDefaultPrevented())
			return!1;
		s.css({
			height : 0,
			overflow : "hidden",
	...
	hide : function(e, a){
		var t, s = $(e);
		if(s.trigger(t = $.Event("hide.ace.submenu")), t.isDefaultPrevented())
			return!1;
		s.css({
			height : e.scrollHeight,
			overflow : "hidden"
...
ace.sidebar_scrollable = function(e, a, t, s, i, n){
	if(e.fn.ace_scroll){
		...
		h = e(window), u = o.get(0), v = l.get(0);
		if(u && v){
			var p, f, g = null, m = null, b = null, w = null, _ = null,
			y = !1, C = !1, a = a || !1, t = t || !1, s = s || !1,
			...
			H = ace.helper.sidebar_scroll = {
					...
					if(!C && x){
						...
						if(C = !0, 1 == h){
							if(H.reset(), a && _.is_active()){
								...
								var T = f.outerHeight();
								k = k.get(0);
								for(var z = f.get(0); z != k;){
									T += z.offsetTop, z = z.parentNode;
								}
								var E = T - g.height();
						...
						if("number" == typeof i && i > 0 && (l.css({"transition-property" : "top", "transition-duration" : (i/1e3).toFixed(2) + "s"}), w.css({"transition-property" : "top", "transition-duration" : (i/1500).toFixed(2) + "s"}), g.on("drag.start", function(e){e.stopPropagation(), l.css("transition-property", "none");}).on("drag.end", function(e){e.stopPropagation(), l.css("transition-property", "top");})), ace.vars.android){
							var S = ace.helper.scrollTop();
							2 > S && (window.scrollTo(S, 0), setTimeout(function(){H.reset()}, 20));
				...
				reset : function(){
					...
					y = !0,
					e && (b.css({height : f, width : 8}), g.prev().css({"max-height" : p}), _.update({size : p}).enable().reset()),
					e && _.is_active() ? o.addClass("sidebar-scroll") : y && H.disable();
				},
				disable : function(){
					y = !1,
					g && (g.css({height : "", "max-height" : ""}), b.css({height : "", width : ""}), g.prev().css({"max-height" : ""}), _.disable()),
					parseInt(v.style.top) < 0 && i && ace.vars.transition ? l.one("transitionend.trans webkitTransitionEnd.trans mozTransitionEnd.trans oTransitionEnd.trans", function(){
			...
			o.on("hidden.ace.submenu shown.ace.submenu", ".submenu", function(e){
				e.stopPropagation(),
				ace.vars.minimized || (ace.vars.webkit ? setTimeout(function(){H.reset();}, 0) : H.reset());
			});
		}
	}
},
...
//		s.on("mouseenter.ace_hover", ".nav-list li.hsub", function(){
	...
	e(document).on("click.ace.widget", ".widget-header a[data-action]", function(t){
		...
		if(0 != i.length && !i.hasClass("ui-sortable-helper")){
			...
			if("collapse" == r){
				...
				"settings" == r && i.trigger("setting.ace.widget");
			}
		}
	});
},
...
ace.settings_box = function(e){
	...
	e("#ace-settings-hover").removeAttr("checked").on("click", function(){
		if(!e(".sidebar").hasClass("h-sidebar")){
			if(this.checked){
				ace.vars["no-scroll"] = !0, e("#sidebar li").addClass("hover").filter(".open").removeClass("open").find("> .submenu").css("display", "none");
			}else{
...
ace.switch_direction = function(e){
	...
	t.toggleClass("rtl")
		...
		.end().find(".pull-right : not(.dropdown-menu,blockquote,.profile-skills .pull-right)")
		.removeClass("pull-right")
		.addClass("tmp-rtl-pull-right")
		.end().find(".pull-left : not(.dropdown-submenu,.profile-skills .pull-left)")
		.removeClass("pull-left")
		.addClass("pull-right")
		.end().find(".tmp-rtl-pull-right")
		.removeClass("tmp-rtl-pull-right")
		.addClass("pull-left")
		.end().find(".chosen-select")
		.toggleClass("chosen-rtl")
		.next().toggleClass("chosen-rtl"),
...
ace.settings_skin = function(e){
	...
	e("#skin-colorpicker").on("change", function(){
		...
		e(".ace-nav > li : not( : last-child)").removeClass("light-pink")
			.find("> a > " + ace.vars[".icon"])
			.removeClass("pink")
			.end().eq(0).find(".badge")
			.removeClass("badge-warning"),
		e(".sidebar-shortcuts .btn").removeClass("btn-pink btn-white")
			.find(ace.vars[".icon"])
			.removeClass("white"),
		e(".ace-nav > li.grey").removeClass("red")
			.find(".badge")
		...
		if("no-skin" == a){
			...
			e(".ace-nav > li").addClass("no-border margin-1"),
			e(".ace-nav > li : not( : last-child)").addClass("light-pink")
				.find("> a > " + ace.vars[".icon"])
				.addClass("pink")
				.end().eq(0).find(".badge")
				.addClass("badge-warning"),
			e(".sidebar-shortcuts .btn").attr("class", "btn btn-white btn-pink")
				.find(ace.vars[".icon"])
				.addClass("white");
		}else if("skin-3" == a){
			t.addClass("no-skin"),
